...
import { UserCircle, Mail, ShieldCheck, CreditCard, Camera, Loader2 } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    // Simulated save — wire to Better Auth updateUser if needed
    ...
    <div className="p-6 md:p-8 max-w-3xl mx-auto">
      ...
      <div className="bg-[#0b1120] border border-[#1e293b] rounded-2xl p-6 mb-4 flex items-center gap-5">
        <div className="relative shrink-0">
          ...
          <div className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-[#6366f1] border-2 border-[#0b1120] flex items-center justify-center cursor-pointer hover:bg-[#5254cc] transition-colors">
          ...
          {!isEditing ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex items-center gap-1.5 text-xs bg-[#6366f1] hover:bg-[#5254cc] text-white px-3 py-1.5 rounded-lg transition-colors disabled:opacity-60"
              >
        ...
        <div className="space-y-4">
          <div>
            ...
            {isEditing ? (
              <input
                ...
                className="w-full bg-[#070f20] border border-[#273448] rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#6366f1]/40 transition-all"
              />
            ) : (
          ...
          <div>
            ...
            <div className="flex items-center gap-2 px-4 py-2.5 bg-[#070f20] border border-[#1e293b] rounded-xl">
              ...
              {user?.emailVerified && (
                <span className="ml-auto text-[10px] text-[#10b981] bg-[#10b981]/10 border border-[#10b981]/20 px-2 py-0.5 rounded-full">
                  Verified
                </span>
              )}
            </div>
      ...
      <div className="bg-[#0b1120] border border-[#1e293b] rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            { label: "Account Role", value: userRole === "admin" ? "Administrator" : "Standard User" },
            { label: "Subscription Plan", value: userPlan === "freeUser" ? "Free Plan" : "Premium Plan" },
            ...
            <div key={item.label} className="bg-[#070f20] border border-[#1e293b] rounded-xl p-4">
