...
function CountUp({ target, suffix }: { target: number; suffix: string }) {
  ...
  return (
    <p className="mt-3 text-4xl font-semibold text-[#1e293b] md:text-5xl">
      {count}
      {suffix}
    </p>
  );
...
export default function Stats() {
  ...
  return (
    <section ref={ref} className="bg-white px-4 py-12 md:px-8 md:py-20 lg:px-16">
      <div className="mx-auto max-w-7xl">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-2 text-3xl font-semibold text-[#1e293b] md:text-4xl">
            The impact is visible in the numbers our members and teams feel every week.
          </h2>
        </div>

        <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label} className="rounded-3xl border border-slate-200 bg-[#f8fafc] p-6">
              {isVisible ? <CountUp target={stat.value} suffix={stat.suffix} /> : <p className="mt-3 text-4xl font-semibold text-[#1e293b] md:text-5xl">0{stat.suffix}</p>}
